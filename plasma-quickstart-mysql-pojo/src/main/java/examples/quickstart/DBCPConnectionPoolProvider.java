...
public class DBCPConnectionPoolProvider implements PoolProvder {
  ...
  public DBCPConnectionPoolProvider() {
    ...
    int poolMinSize = 0;
    String minPoolProp = props
        .getProperty("org.plasma.sdo.access.provider.jdbc.ConnectionPoolMinSize");
    if (minPoolProp != null) {
      try {
    ...
    int poolMaxSize = 10;
    String maxPoolProp = props
        .getProperty("org.plasma.sdo.access.provider.jdbc.ConnectionPoolMaxSize");
    if (maxPoolProp != null) {
      try {
    ...
    try {
      DBCPConnectionPoolProvider.DS = setup(url, user, password, poolMinSize, poolMaxSize, props);
  ...
  private static DataSource setup(String connectURI, String username, String password, int minIdle,
      int maxActive, Properties props) throws Exception {
    //
  ...
  public static void printDriverStats() throws Exception {
    ...
    log.debug("NumActive: " + connectionPool.getNumActive() + " NumIdle: "
